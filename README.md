...
结构化经验增强生成（SE-RAG）在物流异常调度中的应用研究框架。
...
同一异常场景（如车辆故障、交通拥堵、订单取消）下，多个调度方案都可能合理——`delay_tolerant`、`reroute`、`reassign_order` 都是可接受的选项，取决调度员偏好、车队状态、隐性约束。数据中 **~48% 的场景存在多个合法调度方案**，这不是数据质量问题，而是领域固有特征。
...
| 🥉 | no_rag（纯 LLM） | 69.0% | 22.3% | 7.12s | 0.4% |
| 4 | dense_rag | 68.1% | 27.0% | 9.21s | 11.1% |
| 5 | tfidf_rag | 66.0% | 37.3% | 9.40s | 17.6% |

> 全部 1000 queries 统一设置（seed=20260428, deepseek-v4-flash, max_tokens=1024）。
...
3. **48% GT 不一致是领域特征** — 多解评估（Multi-GT）比单一匹配更能反映真实效果
4. **reroute 无退化** — Multi-GT 下单类提升逻辑更清晰
5. **标准 RAG（BM25）错误率高达 15.5%** — 给 LLM 决策增加不确定性；SE-RAG 的错误率仅 2.8%
...
│   ├── generate_validation_queries.py # 验证集抽取
